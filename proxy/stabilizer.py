...
- "Analyze failure -> adapt strategy -> exactly one tool call" reasoning nudge
...
# Conservative trigger set for stabilize mode.
...
# Lightweight nudges based on observed local-model failure modes:
...
def build_retry_payload(
    ...
    *,
    ...
    # Make the steering message prescriptive about tool names: small models do
    # better when the catalog is explicit and they are told not to invent names.
